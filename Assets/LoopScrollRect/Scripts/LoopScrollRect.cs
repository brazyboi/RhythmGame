...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
		...
		[SerializeField]
		private ListViewBaseAdaptor m_listAdapter;
		public ListViewBaseAdaptor listAdapter { get { return m_listAdapter; } set { m_listAdapter = value; 
        ...
        public void RefreshCells()
        {
            if (Application.isPlaying && this.isActiveAndEnabled)
            {
                ...
                {
					if (itemTypeEnd < listAdapter.getTotalCount())
                    {
                        //dataSource.ProvideData(content.GetChild(i), itemTypeEnd);
						m_listAdapter.bindCellData (itemTypeEnd, content.GetChild (i));
                        itemTypeEnd++;
                    }
                    else
                    {
        ...
        public void RefillCellsFromEnd(int offset = 0)
        {
			...
			if (!Application.isPlaying || listAdapter.getTotalCount() < 0 || contentConstraintCount > 1 || listAdapter == null)
                return;
            
            StopMovement();
			itemTypeEnd = reverseDirection ? offset : listAdapter.getTotalCount() - offset;
        ...
        {
			if (!Application.isPlaying || listAdapter == null)
                return;

            StopMovement();
			itemTypeStart = reverseDirection ? listAdapter.getTotalCount() - offset : offset;
            itemTypeEnd = itemTypeStart;
        ...
        {
			if (listAdapter.getTotalCount() >= 0 && itemTypeStart - contentConstraintCount < 0)
            {
                return 0;
            }
            float size = 0;
		...
		{
			SG.ResourceManager.Instance.InitPool(listAdapter.getCellPrefab() , 5);
			return SG.ResourceManager.Instance.GetObjectFromPool(listAdapter.getCellPrefab());
		}

        protected float DeleteItemAtStart()
        {
            // special case: when moving or dragging, we cannot simply delete start when we've reached the end
			if (((m_Dragging || m_Velocity != Vector2.zero) && listAdapter.getTotalCount() >= 0 && itemTypeEnd >= listAdapter.getTotalCount() - 1) 
        ...
        {
			if (listAdapter.getTotalCount() >= 0 && itemTypeEnd >= listAdapter.getTotalCount())
            {
                return 0;
            }
            float size = 0;
        ...
        {
			if (((m_Dragging || m_Velocity != Vector2.zero) && listAdapter.getTotalCount() >= 0 && itemTypeStart < contentConstraintCount) 
                || content.childCount == 0)
            {
                return 0;
            }
        ...
        {            
			RectTransform nextItem = GetPrefabObject().GetComponent<RectTransform>();
            nextItem.transform.SetParent(content, false);
            nextItem.gameObject.SetActive(true);
            //dataSource.ProvideData(nextItem, itemIdx);
			m_listAdapter.bindCellData(itemIdx, nextItem);
			return nextItem;
        }
        //==========LoopScrollRect==========
        ...
        protected override void OnEnable()
        {
            base.OnEnable();
			if (m_listAdapter) {
				m_listAdapter.refreshItems = RefreshCells;
			}
            if (m_HorizontalScrollbar)
                m_HorizontalScrollbar.onValueChanged.AddListener(SetHorizontalNormalizedPosition);
            if (m_VerticalScrollbar)
        ...
        private void UpdateScrollbars(Vector2 offset)
        {
            ...
            {
                //==========LoopScrollRect==========
				if (m_ContentBounds.size.x > 0 && listAdapter.getTotalCount() > 0)
                {
                    m_HorizontalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.x - Mathf.Abs(offset.x)) / m_ContentBounds.size.x * (itemTypeEnd - itemTypeStart) / listAdapter.getTotalCount());
                }
                //==========LoopScrollRect==========
                else
            ...
            {
                //==========LoopScrollRect==========
				if (m_ContentBounds.size.y > 0 && listAdapter.getTotalCount() > 0)
                {
                    m_VerticalScrollbar.size = Mathf.Clamp01((m_ViewBounds.size.y - Mathf.Abs(offset.y)) / m_ContentBounds.size.y * (itemTypeEnd - itemTypeStart) / listAdapter.getTotalCount());
                }
                //==========LoopScrollRect==========
                else
        ...
        public float horizontalNormalizedPosition
        {
            get
            {
                UpdateBounds();
                //==========LoopScrollRect==========
				if(listAdapter.getTotalCount() > 0 && itemTypeEnd > itemTypeStart)
                {
                    //TODO: consider contentSpacing
                    float elementSize = m_ContentBounds.size.x / (itemTypeEnd - itemTypeStart);
					float totalSize = elementSize * listAdapter.getTotalCount();
