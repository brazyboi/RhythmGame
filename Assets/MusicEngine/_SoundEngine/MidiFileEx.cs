...
namespace csmidi
{
    public class MidiFileEx
    {
        public ushort timeDivision;

        public List<MidiTrackEx> midiTracks { get; private set; }

        public MidiFileEx()
        {
            midiTracks = new List<MidiTrackEx>();
            timeDivision = 0;
        }
		...
		public byte[] saveMidiToStream()
		{
			return MidiSaver.saveToStream(midiTracks, timeDivision);         // save the Midi to a file
		}
    ...
    public class MidiTrackEx
    {
        public List<MidiEventEx> midiEvents;

        public MidiTrackEx()
        {
            midiEvents = new List<MidiEventEx>();
        }
    }

    public abstract class MidiEventEx
    {
        private long _ticks;
        ...
        public MidiEventEx(long ticks)
        {
            absoluteTicks = ticks;
        }
    }

    public class MessageMidiEvent : MidiEventEx
    {
    ...
    public class MetaMidiEvent : MidiEventEx
    {
        private byte[] data;
        ...
        /*    byte[] dataLength = VariableLength.ConvertToVariableLength(data.Length);
			...
			returnData[0] = 0xFF;
			returnData[1] = metaType;
			Array.Copy(data, 0, returnData, 2, data.Length);
			return returnData;
        }
    ...
    public class SysExMidiEvent : MidiEventEx
    {
        private byte[] data;
