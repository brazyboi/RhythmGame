...
namespace csmidi
{
    public static class MidiSaver
    {
        public static void saveToFile(string filePath, List<MidiTrackEx> midiTracks, ushort timeDivision)
        {
            ...
            midiWriter.Write(ushortToBigEndian(0));
            ...
            {
                MidiTrackEx cTrk = midiTracks[currentTrack];
		...
		public static byte[] saveToStream(List<MidiTrackEx> midiTracks, ushort timeDivision)
		{
			...
			for (int currentTrack = 0; currentTrack < midiTracks.Count; currentTrack++)
			{
				...
				{
					// write the Delta time to the stream
					long t = cTrk.midiEvents[currentEvent].absoluteTicks - currentTick;
					midiWriter.Write(VariableLength.ConvertToVariableLength(t));
					// write the actual Event data
					midiWriter.Write(cTrk.midiEvents[currentEvent].getEventData());
